...
Problem:
Input three integers day, month, year. Output the date of the next day in DD-MM-YYYY format. Use only if / else logic (no loops). Handle month ends correctly, including leap-year February.
...
If day > maxDay → input invalid (handle as you prefer: print error, or assume valid input per problem).
...
Format output: print with two digits for day and month and four digits for year, e.g. 02-03-2024.
...
(Optional) Invalid input: e.g., 31-04-2023 is invalid because April has 30 days — decide whether to print an error or assume inputs are valid.
...
If you must strictly only use if/else avoid switch or loops — nest conditions carefully and keep readability in mind by using small helper boolean variables (e.g., isLeap).

Print invalid input messages (e.g., "Invalid Date") for out-of-range day/month values — show one test case with invalid input while testing.
...
int main()
{
   ...
   //for ddifferentiating 30 days and 31 days months
